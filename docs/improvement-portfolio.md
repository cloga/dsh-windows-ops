...
| A gateway-backed package duplicated auth, catalog, route, and token concerns already owned by DSH's built-in pi-ai adapter. The standalone plugin now includes the rc.2 Desktop client ModuleLoader handoff, validates Remote authorization results with strict Zod codecs, normalizes provider-owned OAuth grants to strict JSON, shares `llm-pi-ai/github-copilot`, bootstraps authorization on rc.2 while reusing alpha.4's service, materializes account models with their installed per-model API, refreshes through pi-ai, and sends hosted search directly to Copilot. “All-in-one” means one DSH plugin, not an embedded gateway; ACP remains excluded. | [`cloga/dsh-github-copilot#31`](https://github.com/cloga/dsh-github-copilot/pull/31), deployment pin `37261cfe` | **Current locked baseline / 0.3.0-cloga.7** | Artifact SHA-256, client ModuleLoader handoff, strict Remote result codecs, strict JSON OAuth grant normalization, per-model API route materialization, authorization and Zod runtime dependencies, standalone baseline kind, both supported DSH releases, and exactly twelve required capabilities are independently pinned by the Windows operations lock. |
