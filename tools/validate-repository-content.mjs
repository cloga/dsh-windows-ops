...
export function validateRepositoryContent(root = defaultRoot) {
  ...
  if (forkDesktop) {
    expect(desktop?.version === '0.1.6-alpha.1.cloga.16', 'Desktop fork version differs')
    expect(desktop?.source?.releaseTag === 'dsh-desktop-v0.1.6-alpha.1.cloga.16', 'Desktop fork release tag differs')
    expect(desktop?.source?.commit === '2c4f20904887240f83f776c79d8d69a42ce6c1e6', 'Desktop fork source commit differs')
    expect(desktop?.source?.tree === '3329051dfb8ac3681f155f403a902848db13ba37', 'Desktop fork source tree differs')
    expect(desktop?.source?.pullRequest === 107, 'Desktop fork release-plan pull request differs')
    expect(desktop?.source?.reviewedHead === 'eb4789e49bce9e8d9c98b7c71d52c8dde5f20b1d', 'Desktop fork reviewed head differs')
    expect(desktop?.artifact?.name === 'cloga-deepseek-harness-0.1.6-alpha.1.cloga.16-win-x64.exe', 'Desktop fork installer name differs')
    expect(desktop?.artifact?.url === 'https://github.com/cloga/deepseek-harness/releases/download/dsh-desktop-v0.1.6-alpha.1.cloga.16/cloga-deepseek-harness-0.1.6-alpha.1.cloga.16-win-x64.exe', 'Desktop fork installer URL differs')
    expect(desktop?.artifact?.sha256 === '575d74a4c0ac36ee25e8ac604e10f4bc93fbfddf5644ea8eacb4765f4cfdba71', 'Desktop fork installer digest differs')
    expect(desktop?.artifact?.size === 171319113, 'Desktop fork installer size differs')
    expect(desktop?.artifact?.releaseId === 392765616, 'Desktop fork release ID differs')
    expect(desktop?.artifact?.assetId === 578511755, 'Desktop fork installer asset ID differs')
    ...
    expect(desktop?.installedExecutable?.sha256 === '96922b7871947f6a4bbc9c3c5108329dd2030b3b1baff3a8033d9eb98f3f159a', 'installed Desktop fork executable digest differs')
    ...
    expect(desktop?.installedRuntimeDescriptor?.sha256 === 'b5b9b31ce34871ab320f63a91254ba601fd71edc447609d261360d84e4b929e8', 'installed Desktop runtime descriptor digest differs')
    ...
    expect(desktop?.releaseChannel?.sequence === 27, 'Desktop fork release sequence differs')
    expect(desktop?.releaseChannel?.manifestRawSha256 === '6fadba25f0d81b70c3ac2daefd54cb35b9c810bbbf80db35d58c709ad61b8be9', 'Desktop fork manifest raw digest differs')
    expect(desktop?.releaseChannel?.manifestSha256 === '239b9af198a4f854d603db9501e3e307dfeb7baaa4e91a2ee058132c2685cd14', 'Desktop fork manifest self digest differs')
    expect(desktop?.releaseChannel?.buildReceipt?.sha256 === '7897dad36f7cf50d0dffd7f025c35a5ac9743bb2806de5a3ac042e8ed981c09f', 'Desktop fork build receipt raw digest differs')
    expect(desktop?.releaseChannel?.buildReceipt?.receiptSha256 === 'ba59d3c6e899b706dab6030f3309ab9866d6da757decffe0752837952e00e0ad', 'Desktop fork build receipt self digest differs')
    ...
    expect(positiveProof?.schemaVersion === 1 &&
      positiveProof?.sha256 === 'a1515b7ee5af44ff8e7ad86fa07ce8faedaa13f157d02ad99e4a4f99ee174b45' &&
