...
| A gateway-backed package duplicated auth, catalog, route, and token concerns already owned by DSH's built-in pi-ai adapter. The standalone plugin now includes the rc.2 Desktop client ModuleLoader handoff, shares `llm-pi-ai/github-copilot`, bootstraps authorization on rc.2 while reusing alpha.3's service, restricts routes to account-available models, refreshes through pi-ai, and sends hosted search directly to Copilot. “All-in-one” means one DSH plugin, not an embedded gateway; ACP remains excluded. | [`cloga/dsh-github-copilot#23`](https://github.com/cloga/dsh-github-copilot/pull/23), deployment pin `b8b72036` | **Current locked baseline / 0.3.0-cloga.4** | Artifact SHA-256, client ModuleLoader handoff, authorization runtime dependency, standalone baseline kind, both supported DSH releases, and exactly nine required capabilities are independently pinned by the Windows operations lock. |
