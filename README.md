...
| **本地 Core + Desktop + Copilot** | 官方 Desktop 壳优先运行 `cloga/deepseek-harness` 本地 core；`DSH_CLI_PATH` 固定 CLI；Copilot2API 双协议路由；`dsh-web-search-provider` 服务端搜索；可选 loader 依赖缺失的启动恢复；身份边界、验收矩阵和安全升级流程 | `docs/local-core-desktop-copilot.md` |
