...
| A gateway-backed package duplicated auth, catalog, route, and token concerns already owned by DSH's built-in pi-ai adapter. The standalone plugin now includes the rc.2 Desktop client ModuleLoader handoff, validates Remote authorization results with strict Zod codecs, normalizes provider-owned OAuth grants to strict JSON, shares `llm-pi-ai/github-copilot`, bootstraps authorization on rc.2 while reusing alpha.5's service, self-heals absent, empty, incomplete, or stale routes from an existing valid grant, materializes account models with their installed per-model API, keeps Copilot's OpenAI-compatible route on ordinary JSON-schema tool calling so omission-sensitive sandbox arguments remain optional, refreshes through pi-ai, and sends hosted search directly to Copilot. “All-in-one” means one DSH plugin, not an embedded gateway; ACP remains excluded. | [`cloga/dsh-github-copilot#42`](https://github.com/cloga/dsh-github-copilot/pull/42), deployment pin `5417abdb` | **Current locked baseline / 0.3.0-cloga.10** | Release URL and artifact SHA-256, client ModuleLoader handoff, strict Remote result codecs, strict JSON OAuth grant normalization, existing-grant route self-heal, mixed per-model API route materialization, optional tool-argument compatibility, authorization and Zod runtime dependencies, standalone baseline kind, both supported DSH releases, and exactly fourteen required capabilities are independently pinned by the Windows operations lock. |
...
| Partial upgrades mixed Desktop with old Core/plugin and gateway routes. The check-first installer now pins official Desktop 0.10.2, preserves its five 0.6.7 `resources\node_modules` plugin junctions, migrates reviewed gateway/route/reference state with backup, and accepts only the built-in pi-ai grant plus reference-free direct route. | `cloga/dsh-windows-ops#53`, `cloga/dsh-windows-ops#72` | **Current deployment baseline** | Combined installer/bootstrap Pester suites, plugin catalog tests, validator, replay self-check, artifact hash verification, and default read-only check define the verification gate. |
