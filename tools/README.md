...
| Check or install the pinned local Desktop side-by-side | `powershell.exe -NoProfile -ExecutionPolicy Bypass -File tools\install-official-desktop-local.ps1 [-Apply -AcknowledgeUnsignedLocalBuild]` | None in default Check; explicit Apply packages, installs locally, and writes isolated launcher/shortcuts/receipt | Revalidates the fresh `PackageLocal` receipt and artifact bytes, refuses running local-build processes, uses NSIS `/S` plus destination-last `/D=`, preserves the community Desktop, never launches/restarts/uninstalls, and reports post-installer failure as partial/manual review. See [local build and install guide](../docs/official-desktop-local-build.md) |
