...
| A gateway-backed package duplicated auth, catalog, route, and token concerns already owned by DSH's built-in pi-ai adapter. The standalone plugin now includes the rc.2 Desktop client ModuleLoader handoff, validates Remote authorization results with strict Zod codecs, normalizes provider-owned OAuth grants to strict JSON, shares `llm-pi-ai/github-copilot`, bootstraps authorization on rc.2 while reusing alpha.5's service, self-heals absent, empty, incomplete, or stale routes from an existing valid grant, materializes account models with their installed per-model API, removes unusable escalation fields only from Copilot tool schemas while preserving native schemas for every other provider, refreshes through pi-ai, and sends hosted search directly to Copilot. “All-in-one” means one DSH plugin, not an embedded gateway; ACP remains excluded. | [`cloga/dsh-github-copilot#45`](https://github.com/cloga/dsh-github-copilot/pull/45), deployment pin `7db12efa` | **Current locked baseline / 0.3.0-cloga.11** | Release URL and artifact SHA-256, client ModuleLoader handoff, strict Remote result codecs, strict JSON OAuth grant normalization, existing-grant route self-heal, mixed per-model API route materialization, Copilot-scoped escalation-field removal, authorization and Zod runtime dependencies, standalone baseline kind, both supported DSH releases, packaged current/fresh Session schema-filter proofs, and exactly fourteen required capabilities are independently pinned by the Windows operations lock. |
