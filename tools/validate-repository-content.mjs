...
export function validateRepositoryContent(root = defaultRoot) {
  ...
  expect(artifact?.releaseCommit === 'b75eac570cd418497c52e80a3ce47958cdcc6b26', 'locked Copilot Release commit differs')
  ...
  if (forkDesktop) {
    expect(desktop?.version === '0.1.6-alpha.1.cloga.12', 'Desktop fork version differs')
    expect(desktop?.source?.releaseTag === 'dsh-desktop-v0.1.6-alpha.1.cloga.12', 'Desktop fork release tag differs')
    expect(desktop?.source?.commit === 'd19be3ff5524948c40cb9929cdd4d67d5cb35059', 'Desktop fork source commit differs')
    expect(desktop?.source?.tree === '681980cf0d3448679529628477d9ff79ea927cc4', 'Desktop fork source tree differs')
    expect(desktop?.source?.pullRequest === 87, 'Desktop fork release-plan pull request differs')
    expect(desktop?.source?.reviewedHead === '3e277c3f0821264d4becca825636b5fcd8112bcd', 'Desktop fork reviewed head differs')
    expect(desktop?.artifact?.name === 'cloga-deepseek-harness-0.1.6-alpha.1.cloga.12-win-x64.exe', 'Desktop fork installer name differs')
    expect(desktop?.artifact?.url === 'https://github.com/cloga/deepseek-harness/releases/download/dsh-desktop-v0.1.6-alpha.1.cloga.12/cloga-deepseek-harness-0.1.6-alpha.1.cloga.12-win-x64.exe', 'Desktop fork installer URL differs')
    expect(desktop?.artifact?.sha256 === 'f1fadab2922b9a2f56d72e07b3072c8b871d5ccce9696b82fcc62c22824fcd16', 'Desktop fork installer digest differs')
    expect(desktop?.artifact?.size === 171323022, 'Desktop fork installer size differs')
    expect(desktop?.artifact?.releaseId === 392534651, 'Desktop fork release ID differs')
    expect(desktop?.artifact?.assetId === 577349351, 'Desktop fork installer asset ID differs')
    ...
    expect(desktop?.installedExecutable?.sha256 === 'ef6d3b63d2e495d7fb421327c5f7d9fb48970dbb44f55b1ff245890b52d4dada', 'installed Desktop fork executable digest differs')
    ...
    expect(desktop?.installedRuntimeDescriptor?.sha256 === '3e20cd0ace516569c9ead2403acffe16ed0a3e99b1ef45d17e0bca3cc7bd06f3', 'installed Desktop runtime descriptor digest differs')
    ...
    expect(desktop?.releaseChannel?.sequence === 22, 'Desktop fork release sequence differs')
    expect(desktop?.releaseChannel?.manifestRawSha256 === '7b6d0ff3394f72b8d50587a7f014b9c0bcbfc42570f7fee0bfbc5e4f7de4a7b1', 'Desktop fork manifest raw digest differs')
    expect(desktop?.releaseChannel?.manifestSha256 === '4c76bdd67a0f021cc6375418d985877a9da82794f5dd1b037c2ea63ffe3d5bea', 'Desktop fork manifest self digest differs')
    expect(desktop?.releaseChannel?.buildReceipt?.sha256 === '160e30810f24314d539de2ce7eeb25c81b527f558b10bfbe9972595fe69e7165', 'Desktop fork build receipt raw digest differs')
    expect(desktop?.releaseChannel?.buildReceipt?.receiptSha256 === 'd0b74b9a1ba42dcd31a2c0c46ac4d060c2944fc45ebec529023efb5a6d1b08e0', 'Desktop fork build receipt self digest differs')
    try {
