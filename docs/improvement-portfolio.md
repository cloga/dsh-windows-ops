...
| A gateway-backed package duplicated auth, catalog, route, and token concerns already owned by DSH's built-in pi-ai adapter. The standalone plugin now includes the rc.2 Desktop client ModuleLoader handoff, validates Remote authorization results with strict Zod codecs, shares `llm-pi-ai/github-copilot`, bootstraps authorization on rc.2 while reusing alpha.3's service, restricts routes to account-available models, refreshes through pi-ai, and sends hosted search directly to Copilot. “All-in-one” means one DSH plugin, not an embedded gateway; ACP remains excluded. | [`cloga/dsh-github-copilot#26`](https://github.com/cloga/dsh-github-copilot/pull/26), deployment pin `5d8d696a` | **Current locked baseline / 0.3.0-cloga.5** | Artifact SHA-256, client ModuleLoader handoff, strict Remote result codecs, authorization and Zod runtime dependencies, standalone baseline kind, both supported DSH releases, and exactly ten required capabilities are independently pinned by the Windows operations lock. |
