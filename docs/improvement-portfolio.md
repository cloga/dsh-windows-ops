...
| A gateway-backed package duplicated auth, catalog, route, and token concerns already owned by DSH's built-in pi-ai adapter. The current companion reuses that owner while adding the official Desktop client handoff, strict Remote codecs and Host-only grant normalization, authorization fallback, account-aware leaf-only model reconciliation, mixed per-model protocols, provider-scoped Tool Schema filtering, Responses/Anthropic inline search, Responses-only `ctx.web` search, explicit probe-bypass semantics, and validated GitHub-hosted/Enterprise endpoints. Missing profiles are created without connection references; existing profiles preserve unowned fields, while the Windows deployment removes reviewed legacy references. “All-in-one” means one DSH plugin, not an embedded gateway; ACP remains excluded. | [`cloga/dsh-github-copilot` alpha.24](https://github.com/cloga/dsh-github-copilot/releases/tag/v0.4.0-alpha.24), immutable source `e49bf7c9307cf22dd9ea720bed8750101fc986ed` | **Current paired baseline / immutable Release 0.4.0-alpha.24** | Exact Release/source/artifact/checksum and provisioning contracts remain pinned. Alpha.24 fixes alpha.23's missing Client `remote.githubCopilotSearchRouting` injection. Paired Desktop `0.1.6-alpha.1.cloga.2` / Core `.6` formal run `35271210350` records actual initial/restart Model roles/search-routing DOM and read-only registered-provider catalogs. Genuine native Ops run `35278350619` passed at `243c33d286f19d9e4c608e238a52de2b9a136b9e` with independently byte-bound summary; earlier pre-observer failure `35276462350` remains unexplained. No whole-carrier, real search/OAuth/model round or local upgrade/activation is claimed. |
