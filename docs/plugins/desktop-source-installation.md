...
The later `.cloga.7` formal run `35178158751` built and published on attempt 1. Only remote-check initially received HTTP 403; the failed-job-only rerun succeeded on attempt 2 without an artifact rebuild or overwrite. Archived formal acceptance artifact `10479574730` establishes isolated packaged Electron initial/restart Copilot Models account UI, graph/ancestor isolation, and standalone helper ACK/cancel. It does **not** establish OAuth, a model round, a local installer upgrade, or an installed end-to-end Memory Evolve/source-plugin workflow. Exact published artifact hashes are recorded in the [baseline guide](../local-core-desktop-copilot.md#authoritative-baseline); downloaded-byte verification did not execute the installer or change local Desktop.
