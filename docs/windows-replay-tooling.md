...
The `0.4.0-alpha.32` baseline checks:
...
| Shared routing chunk | exact alpha.32 published entrypoints/shared routing chunk are pinned; preserve canonical route API precedence, provider-only CAS and hostname-gated request metadata |
...
| Desktop-managed DSH | bundled `@deepseek-ai/dsh@0.1.6-alpha.1`, virtual descriptor SHA-256 `8cee8fcf841cc28235bc561aa89082e910bfc648cbfc953f735910bc19bfbe55`; audited virtual plus unpacked backing inventory, no separate Core |
| Desktop | fork-owned `0.1.6-alpha.1.cloga.14`, sequence 24, source `a0f0144f4cddc90c45f8be93c61c0cd6cec470c7`; formal source acceptance and registered native Ops run `35553019059` passed (no local activation) |
...
The authoritative plugin is `dsh-github-copilot@0.4.0-alpha.32`, immutable
Release source `76d190aed688e073df930adb3c753d2a749519c9`; artifact SHA-256 is
...
Current `.cloga.14` formal run `35549412610` independently bound six Release assets and 70 Actions ZIP entries. Schema-2 initial/restart evidence covers read-only workspace/catalog, provider-only routing/Fallback label and exact Desktop version menu; schema-1 usage evidence binds absent signed-out quota UI after account readiness without Host request instrumentation. Registered native Ops run `35553019059` passed at exact head `68a1218ef6a50f06870bae483d64f52d6de244aa`; it proves no OAuth, same-window navigation, provider availability, live quota or real search, and older summaries cannot be transferred.
