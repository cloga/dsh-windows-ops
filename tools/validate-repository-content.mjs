...
export function validateRepositoryContent(root = defaultRoot) {
  ...
  if (forkDesktop) {
    expect(desktop?.version === '0.1.5-rc.3.cloga.7', 'Desktop fork version differs')
    expect(desktop?.source?.releaseTag === 'dsh-desktop-v0.1.5-rc.3.cloga.7', 'Desktop fork release tag differs')
    expect(desktop?.source?.commit === '293b5a79f533005d99cd60cb00b9ecf810187401', 'Desktop fork source commit differs')
    expect(desktop?.source?.tree === 'de807cc4c582f3bb6e2829ea98758e98ec5e576c', 'Desktop fork source tree differs')
    expect(desktop?.source?.pullRequest === 57, 'Desktop fork release-plan pull request differs')
    expect(desktop?.source?.reviewedHead === 'acf99f2038c42cafa076d4aff69f01f43260c988', 'Desktop fork reviewed head differs')
    expect(desktop?.artifact?.name === 'cloga-deepseek-harness-0.1.5-rc.3.cloga.7-win-x64.exe', 'Desktop fork installer name differs')
    expect(desktop?.artifact?.url === 'https://github.com/cloga/deepseek-harness/releases/download/dsh-desktop-v0.1.5-rc.3.cloga.7/cloga-deepseek-harness-0.1.5-rc.3.cloga.7-win-x64.exe', 'Desktop fork installer URL differs')
    expect(desktop?.artifact?.sha256 === '290b587cdab3ffa315ba1796ccabd436096f996abaa144652e33457fd65a3b93', 'Desktop fork installer digest differs')
    expect(desktop?.artifact?.size === 180217778, 'Desktop fork installer size differs')
    expect(desktop?.artifact?.releaseId === 390421989, 'Desktop fork release ID differs')
    expect(desktop?.artifact?.assetId === 569397800, 'Desktop fork installer asset ID differs')
    ...
    expect(desktop?.installedExecutable?.sha256 === '1bf4524a0779f4451d9371d40a09fb0b9a1b44e0881818afd73809a771ac7711', 'installed Desktop fork executable digest differs')
    ...
    expect(desktop?.installedRuntimeDescriptor?.sha256 === 'ccb45e7c151318c0f239de2dafa64dcec05b8fdff6347cfbc7ed3b2de3264092', 'installed Desktop runtime descriptor digest differs')
    ...
    expect(desktop?.releaseChannel?.sequence === 9, 'Desktop fork release sequence differs')
    expect(desktop?.releaseChannel?.manifestRawSha256 === 'cacc56fa4b5f4cd9e8388a85e147034f63398f0efa4203b175e4bc6eeaf166c9', 'Desktop fork manifest raw digest differs')
    expect(desktop?.releaseChannel?.manifestSha256 === 'fd9952367bc74a0c25d803330d185c1ce302eb0c1e587ba5b0669198b6cd6178', 'Desktop fork manifest self digest differs')
    expect(desktop?.releaseChannel?.buildReceipt?.sha256 === '697c937714a89e7d58fe2fc0096681ee11a493f16174e3c02e18de1eca87c8e7', 'Desktop fork build receipt raw digest differs')
    expect(desktop?.releaseChannel?.buildReceipt?.receiptSha256 === 'af982a0f5e14b0ea20f41c22d0a8ee5c0fb0d8db316626f6feb9ae18e9f69e2d', 'Desktop fork build receipt self digest differs')
    try {
