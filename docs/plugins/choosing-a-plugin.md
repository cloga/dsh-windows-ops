...
Because the reviewed Release contains no immutable package asset, this repository does not publish a verified Release artifact hash or recommend a production installation command for it. The [Desktop source-snapshot guide](desktop-source-installation.md#evidence-recorded-for-issue-50) separately records pre-merge acquisition, packing, and validation of this exact source commit without Host activation. The installation capability is now formally published in immutable Desktop `0.1.5-rc.3.cloga.7` (sequence 9), but this does not mean it has been locally installed or Memory Evolve activated. Its original local packed-byte hash is not an upstream Release checksum, and neither that evidence nor Desktop publication promotes the catalog's `L1`/`experimental` classification. A source import alone is not the `L2` host-entry probe defined by this catalog.
