...
| A gateway-backed package duplicated auth, catalog, route, and token concerns already owned by DSH's built-in pi-ai adapter. The current companion delegates those official primitives while retaining account-owned discovery, provider-only routing/fallback, strict codecs/grant normalization, Tool Schema filtering and search proof/cancellation gaps. Alpha.33 fixes `useSession` with the official selector contract, without a Core change. “All-in-one” means one DSH plugin, not an embedded gateway or official Core; ACP remains excluded. | [`cloga/dsh-github-copilot` alpha.33](https://github.com/cloga/dsh-github-copilot/releases/tag/v0.4.0-alpha.33), immutable source `aa90fe434da8b2172faa1446afa0a0fd006afe00`, PR #157 | **Current published pair: Copilot 0.4.0-alpha.33 / Desktop .cloga.17 / sequence 28; fresh hosted Native Ops run 35595040585 attempt 1 SUCCESS at exact code head bb7a0a366e789b19918be6d8a6e40266c46a94f9** | Current runtime-only Desktop maintenance preserves Core `0.1.6-alpha.1` and Copilot alpha.33; formal run `35583602522` attempt 1 SUCCESS and immutable Release `392847203` are independently verified. See [the owning .17 record](desktop-external-links-17.md) and [native provenance](../tests/fixtures/desktop-native-verified-release/ops-cloga016-17/README.md). Native job `106318363183` and all four jobs passed; later evidence commits/merges are not the qualified head. First native failure `35590167413` retains an unknown startup cause, not a diagnostic repair claim; ordinary CI `35593395998` was a separate LF/CRLF test-extraction defect fixed test-only at `bb7a0a3…`. No local installation/activation/restart, Core alpha.2 or Copilot alpha.34 adoption is authorized. **Historical .16 evidence only:** Desktop `.cloga.16` / sequence 27 / unchanged bundled Core alpha.1 formal run `35569892548` attempt 2 succeeded. Schema-2 workspace/catalog/routing/menu and schema-1 signed-out usage proof remain required. Authenticated optional positive proof uses actual renderer/released Client/SessionProvider/Slot but synthetic Session/quota/test mount, canonical+preview once after restart graph with no Host transport. Fresh positive proof is enforced by the exact qualified `a59f586df4e329c3bc8b3f885013fdb5493f4970` driver, not new summary flags. The later documentation/evidence-only follow-up records that code-head result, not its own native qualification. Whole-carrier/model-response/installer-upgrade flags remain false. Historical alpha.32 run `35553019059` is not transferred; formal release run `35569892548` attempt 1's `EBUSY` cleanup failure remains history. Stage-only was chosen; live account verification awaits a future human request, without local install/activation/restart. [Exact proof and hashes](copilot-alpha33-upgrade-record.md). Alpha.2 compatibility is not Core promotion; no live account/network, OAuth/model/search or local upgrade/activation is claimed. |
...
### Historical private `deepseek-harness` work (retired from deployment)

| Root cause and validated behavior | Owning change and upstream design record | Status | Validation evidence |
|---|---|---|---|
| Standard OpenAI model discovery discarded useful gateway metadata or over-filtered minimal catalogs. Discovery propagated optional picker, policy, endpoint, token-limit, and reasoning metadata without requiring extensions. | `cloga/deepseek-harness`; upstream [Discussion #4523](https://github.com/deepseek-ai/deepseek-harness/discussions/4523) | **Historical fork work** | Focused tests 158/158; typecheck, build, lint, docs sync, pre-push build, and contract typecheck passed. |
| Onboarding assumed every provider required the same credential, leaving valid custom/OpenAI-compatible configurations blocked or marked unhealthy. Readiness derived from usable configured providers while retaining real missing-credential errors. | `cloga/deepseek-harness`; upstream [Discussion #4537](https://github.com/deepseek-ai/deepseek-harness/discussions/4537) | **Historical fork work** | Focused tests 93/93, onboarding replay E2E 3/3, complete GUI suite, typecheck, lint, docs sync, and production build passed. |
| Discovered image capability stopped before the registry, RPC, profile, and editor surfaces. Optional gateway vision metadata became `text + image`; absent metadata remained safely text-only. | `cloga/deepseek-harness`; upstream [Discussion #4524](https://github.com/deepseek-ai/deepseek-harness/discussions/4524) | **Historical fork work** | Focused tests 157/157; relevant GUI suite 3,997 passed with 1 skipped; typecheck, build, lint, docs sync, and models-settings E2E passed. |
| The private Core lacked a complete, provenance-bearing local release installation and a stable Desktop-facing shim. | `cloga/deepseek-harness` | **Historical; no longer selectable** | Complete release closure pack/install, receipt hashes, isolated Web boot, and Desktop shim validation passed. |
| Same-mode or narrower sandbox requests were rejected as invalid escalation. The retired fork retained effective policy without approval and asked once only for strict widening. | `cloga/deepseek-harness`, historical base `bd520d6e` and pin `a772dbbd` | **Historical maintenance work** | Focused bash/pwsh/shared 154/154 and fs 74/74; typecheck, lint, official build, release pack, installed-file attestation, and sandbox probe passed. |
| pi-ai OAuth grants could contain own `undefined` members that cannot survive credential JSON/YAML persistence. The historical adapter rebuilt detached strict JSON and rejected cyclic or non-JSON values without disclosure. | `cloga/deepseek-harness`, historical branch `cloga-pi-ai-oauth-json-records`; [`cloga/dsh-windows-ops#65`](https://github.com/cloga/dsh-windows-ops/issues/65) | **Historical; retired from baseline** | Historical credential-store regressions covered recursive normalization, exact round-trip, rejection categories, and redacted diagnostics. |
| A provider route could select only one wire protocol, so mixed Copilot catalogs could not serve Responses and Chat Completions models together. Historical work validated model-level `api` precedence. | `cloga/deepseek-harness`, historical branch `cloga-pi-ai-model-api`; [`cloga/dsh-windows-ops#68`](https://github.com/cloga/dsh-windows-ops/issues/68) | **Historical; retired from baseline** | Historical schema, catalog-precedence, streaming, and built-marker tests covered mixed-protocol routes. |

These entries are retained as historical design and validation records only.
The current deployment neither builds nor selects this private runtime. The
linked upstream Discussions are not merged code or release commitments.
...
|---|---|---|---|
| Host-wide isolated Edge testing through `dsh-playwright-host@0.1.7` | [`cloga/dsh-playwright-host@v0.1.7`](https://github.com/cloga/dsh-playwright-host/releases/tag/v0.1.7) | **Reviewed immutable optional overlay** | Source and artifact bytes verified for Core 0.1.6-alpha.1; no new local activation or browser smoke claimed. Not a native baseline requirement; one Host MCP process is shared across Sessions. |
| Session-bound scheduling through `dsh-cron@0.7.1` | [`cloga/dsh-cron@v0.7.1`](https://github.com/cloga/dsh-cron/releases/tag/v0.7.1) | **Reviewed immutable optional overlay** | Source and artifact bytes verified for Core 0.1.6-alpha.1; no new local activation or scheduler smoke claimed. Web-only; Windows native notification flags remain documented quiet no-ops. |
| Former model-aware text-only vision-tool fallback | [`tianmingwan/dsh-vision-any#2`](https://github.com/tianmingwan/dsh-vision-any/pull/2) | **Historical / superseded** | Not installed or recommended by the Windows Copilot lock. Image-capable routes use DSH's native attachment path; workspace files use built-in `read_image`. |
