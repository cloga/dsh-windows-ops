...
| A gateway-backed package duplicated auth, catalog, route, and token concerns already owned by DSH's built-in pi-ai adapter. The standalone plugin now includes the rc.2 Desktop client ModuleLoader handoff, validates Remote authorization results with strict Zod codecs, normalizes provider-owned OAuth grants to strict JSON, shares `llm-pi-ai/github-copilot`, bootstraps authorization on rc.2 while reusing alpha.4's service, self-heals absent, empty, incomplete, or stale routes from an existing valid grant, materializes account models with their installed per-model API, refreshes through pi-ai, and sends hosted search directly to Copilot. “All-in-one” means one DSH plugin, not an embedded gateway; ACP remains excluded. | [`cloga/dsh-github-copilot#33`](https://github.com/cloga/dsh-github-copilot/pull/33), deployment pin `94d921dc` | **Current locked baseline / 0.3.0-cloga.8** | Release URL and artifact SHA-256, client ModuleLoader handoff, strict Remote result codecs, strict JSON OAuth grant normalization, existing-grant route self-heal, mixed per-model API route materialization, authorization and Zod runtime dependencies, standalone baseline kind, both supported DSH releases, and exactly thirteen required capabilities are independently pinned by the Windows operations lock. |
