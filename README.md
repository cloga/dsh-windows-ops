...
| [`cloga/dsh-github-copilot`](https://github.com/cloga/dsh-github-copilot) | 一个包含 rc.2 Desktop 客户端 ModuleLoader handoff 和严格 Remote 结果 codec 的 DSH 插件，复用内置 `@deepseek-ai/dsh-llm-pi-ai` 的 OAuth、账户模型、刷新、Copilot 直连和 hosted search；在 rc.2 中启动 authorization service，并在 alpha.3 中复用已有服务；不包含 ACP | 部署精确 pin `5d8d696a` / `0.3.0-cloga.5` |
