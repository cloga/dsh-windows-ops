...
下一次昂贵运行前，按[有界发布调试清单](docs/core-upgrade.md#bounded-release-debugging)
冻结候选范围、收齐同轮失败、检查真实 UI／原生前置条件，并分开记录各验收阶段。
这些是流程经验，不代表新版本合格、已建立制品复用流程或获得本机激活权限。

本次 [0.1.6-alpha.2 官方对照与迁移决定](docs/core-016a2-assessment.md)记录已审视的
官方替代、必须保留的差异和已发现的接口问题；源码评估不是新部署基线。
后续合并、发布、哈希与验收范围见[适配交付证据](docs/core-016a2-delivery.md)；Cron、Playwright 与 Copilot alpha.28 的历史发布证据继续保留。当前已发布 Ops 目标为已独立核验的 Desktop `0.1.6-alpha.1.cloga.18` / sequence 30 / bundled Core alpha.1 / Copilot alpha.35，见[原生 composer 交付记录](docs/desktop-inline-composer-18.md)。正式 run `35661128403` attempt 1 的三个 jobs（含 remote Check）成功；新的独立 hosted Native Ops 验收仍为 **PENDING**。Core 版本不变，仅含明确授权的 Core Client 布局变更；另行负责的 alpha.2 Desktop 尚未合格。本机不安装、激活或重启，也不宣称真实 compaction、OAuth、model 或 search 验证。
...
当前已发布 Ops 目标 Desktop `.cloga.18` 仍内置 Core alpha.1；历史 Alpha.33 的 alpha.2 admission
...
此 alpha.2 准备工作不修改六资产公开契约，也不改变当前 `.cloga.18`/alpha.35
目标的 Core alpha.1 范围；该目标新的独立 hosted Native Ops 验收仍为 **PENDING**。
历史 `.cloga.17` 的[外链记录](docs/desktop-external-links-17.md)与 `.cloga.16` 的
[正式/Ops 证据](docs/copilot-alpha33-upgrade-record.md)不可移用。
Alpha.2 尚未发布、尚未合格；此准备工作不授权安装、激活或重启。
...
## 当前已发布 Ops 部署目标（独立 Native Ops 验收待完成）

机器可执行契约以 [`deployments/windows-copilot.lock.json`](deployments/windows-copilot.lock.json) 为准。发布制品、正式证据及原始 ZIP 字节绑定已于 **2026-09-21** 由父代理独立核验；新的 hosted Native Ops 验收仍为 **PENDING**，不继承历史成功。

| 组件 | 目标身份 |
|---|---|
| DeepSeek Harness Desktop | fork-owned `0.1.6-alpha.1.cloga.18`，sequence 30，不可变 Release `393339432`，source `202dd0a2022da939aa424ac1714cd12ce6d902ef` |
| Desktop 管理的 DSH runtime | bundled `@deepseek-ai/dsh@0.1.6-alpha.1`；descriptor SHA-256 `15dd038067c58882e2efa6f4465e4f7bcf98f86612e4d5a484d01286080b329d`；ASAR root 跟随实际 EXE |
| 必需的 `dsh-github-copilot` | 不可变 `0.4.0-alpha.35`，source `6554417dc9a7544865e6c1bbdebf8b9a10e0a7af`；由 `desktopNativeVerifiedRelease` 接管 |
| 正式 packaged 证据 | [run `35661128403`](https://github.com/cloga/deepseek-harness/actions/runs/35661128403) attempt 1，三个 jobs **SUCCESS**（含 remote Check）；settings schema 3、signed-out usage、positive usage 与 native composer |
| Native Ops qualification | **PENDING**；需对当前精确 Ops head 单独执行，不生成虚构 summary flags |

[交付记录](docs/desktop-inline-composer-18.md)保存精确六资产及验收边界。Alpha.35 保留 alpha.34
的 Model roles 退役，隐藏不可用 Session credits 和无效 reset 日期；不恢复旧 role UI。
真实 InputBar/StatsPills/released Client 的 1280/400 几何与弹窗使用合成持久历史、无凭据
signed-out Host；独立 positive fixture 使用合成 Session/quota 且无 Host transport。
当前 settings schema 3 使用 `accountViewLoaded`/`retiredModelRolesAbsent`，不移用历史 schema 2
的 workspace/role 声明。跨次检查动态几何语义，同次证据严格关联，不要求跨次像素哈希相等。
历史 [`.cloga.17` 发布与 Native Ops 记录](docs/desktop-external-links-17.md)保留，不转作当前验收。
保持 **stage-only**，不在本机安装、激活或重启；真实账号验证另待人类请求。
...
Lock 更新表示经过评审的目标，不代表本机状态；`.cloga.18` 的独立 hosted Native Ops 验收仍为 **PENDING**。已安装 Desktop 仍为 `.cloga.10`
