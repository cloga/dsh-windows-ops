...
const alphaRoot = path.resolve(process.env.DSH_016_ROOT)
const pluginRoot = path.resolve(process.env.DSH_DEV_TOOLS_ROOT)
const fromAlpha = (relative) => pathToFileURL(path.join(alphaRoot, ...relative.split('/'))).href

const [{ Context }, { default: SystemPrompt }, { default: ToolRuntime }, { default: SubprocessRuntime }, plugin] = await Promise.all([
  import(fromAlpha('vendor/cordis/src/index.ts')),
  import(fromAlpha('packages/core/system-prompt/src/index.ts')),
  import(fromAlpha('packages/core/tools/src/index.ts')),
  import(fromAlpha('packages/subprocess/subprocess/src/index.ts')),
...
class MockSubprocessRuntime extends SubprocessRuntime {
  ...
  async terminalEnvironment() { return { platform: 'windows', defaultShell: 'pwsh' } }
