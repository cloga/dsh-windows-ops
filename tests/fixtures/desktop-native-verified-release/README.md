...
| [`ops-cloga016-1/`](ops-cloga016-1/README.md) | Separate successful genuine Ops observer summary from run `35210215981`, qualified code head `83b0303c250b62f55424be3d88347bf147c593d8` |
...
These are **actual formal release/source-acceptance evidence**, not rehearsal data. By themselves they do not establish Windows Ops observer qualification. The separate raw summary in `ops-cloga016-1/` records that successful genuine check at code head `83b0303c250b62f55424be3d88347bf147c593d8` / run `35210215981`; its ZIP and sole entry bytes were independently checked. The eleven formal files above remain unchanged. No local Desktop installation/upgrade, restart, real OAuth or model round is claimed. Runtime descriptor `schemaVersion` is 1; nested `release.hostProtocolVersion` is 3. The logical `app.asar/dsh` package/file pins come from the separately hash-attested formal runtime descriptor; historical physical-wrapper hashes are not a `.6` wrapper identity.
