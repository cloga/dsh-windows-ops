...
| Check or install the pinned local Desktop side-by-side | `powershell.exe -NoProfile -ExecutionPolicy Bypass -File tools\install-official-desktop-local.ps1 [-Apply -AcknowledgeUnsignedLocalBuild] [-SharedHome <existing-home>\|-UseIsolatedHome]` | None in default Check; explicit Apply packages and installs locally, then writes the owned launcher/shortcuts/receipt | New installs use an isolated Harness home by default. `-SharedHome` explicitly reuses one existing Home while Electron user data remains isolated; schema-2 receipts persist the selection and `-UseIsolatedHome` switches the launcher back without copying or deleting shared data. Revalidates `PackageLocal`, refuses live consumers, preserves the community Desktop, and never launches/restarts/uninstalls. See [local build and install guide](../docs/official-desktop-local-build.md) |
| Package, check, stage, or complete an operations-owned local Desktop update | `powershell.exe -NoProfile -ExecutionPolicy Bypass -File tools\manage-official-desktop-update-channel.ps1 -Action Package\|Check\|Stage\|Complete ...` | Package writes only below the safe build root; Check is read-only; Stage copies an acknowledged bundle; Complete writes attested schema-3 receipt/owned launcher metadata only after the user manually ran the installer | Emits `rc.yml`, strict SHA-256/SHA-512 inventory, and a self-hashed manifest. It never publishes, embeds `app-update.yml`, bypasses signature checks, runs the unsigned installer, stops processes, or claims rollback. See [managed update channel](../docs/official-desktop-local-build.md#dsh-windows-ops-managed-update-channel-unsigned-manual-apply) |
