...
| A gateway-backed package duplicated auth, catalog, route, and token concerns already owned by DSH's built-in pi-ai adapter. The current companion delegates those official primitives while retaining account-owned discovery, provider-only routing/fallback, strict codecs/grant normalization, Tool Schema filtering and search proof/cancellation gaps. “All-in-one” means one DSH plugin, not an embedded gateway or official Core; ACP remains excluded. | [`cloga/dsh-github-copilot` alpha.32](https://github.com/cloga/dsh-github-copilot/releases/tag/v0.4.0-alpha.32), immutable source `76d190aed688e073df930adb3c753d2a749519c9` | **Current paired baseline / immutable Release 0.4.0-alpha.32** | Desktop `.cloga.14` / bundled Core alpha.1 formal run `35549412610` records schema-2 read-only workspace/catalog, provider-only routing/Fallback labels, exact version menus and schema-1 signed-out usage evidence across restart. Registered native run `35553019059` passed at exact Ops head `68a1218ef6a50f06870bae483d64f52d6de244aa`. Alpha.2 compatibility is not Core promotion; no Host packet observation, live quota, external-navigation success, real search/OAuth/model round or local upgrade/activation is claimed. |
