...
export function validateRepositoryContent(root = defaultRoot) {
  ...
  expect(artifact?.releaseCommit === 'e49bf7c9307cf22dd9ea720bed8750101fc986ed', 'locked Copilot Release commit differs')
  ...
  if (forkDesktop) {
    expect(desktop?.version === '0.1.6-alpha.1.cloga.2', 'Desktop fork version differs')
    expect(desktop?.source?.releaseTag === 'dsh-desktop-v0.1.6-alpha.1.cloga.2', 'Desktop fork release tag differs')
    expect(desktop?.source?.commit === '65a236bd65f2971f98b11a0efd020b8860144924', 'Desktop fork source commit differs')
    expect(desktop?.source?.tree === 'b219bd1baa93433e9449dc72905d7980e7943a05', 'Desktop fork source tree differs')
    expect(desktop?.source?.pullRequest === 63, 'Desktop fork release-plan pull request differs')
    expect(desktop?.source?.reviewedHead === 'c8af5b6cb3ccf651a5ff1a285ff7bf0ac95f487a', 'Desktop fork reviewed head differs')
    expect(desktop?.artifact?.name === 'cloga-deepseek-harness-0.1.6-alpha.1.cloga.2-win-x64.exe', 'Desktop fork installer name differs')
    expect(desktop?.artifact?.url === 'https://github.com/cloga/deepseek-harness/releases/download/dsh-desktop-v0.1.6-alpha.1.cloga.2/cloga-deepseek-harness-0.1.6-alpha.1.cloga.2-win-x64.exe', 'Desktop fork installer URL differs')
    expect(desktop?.artifact?.sha256 === 'cf140d49b8df9096b52fba365066ef4eeee06eed57ca0f16c2fc319f1e5f0970', 'Desktop fork installer digest differs')
    expect(desktop?.artifact?.size === 171301229, 'Desktop fork installer size differs')
    expect(desktop?.artifact?.releaseId === 391052820, 'Desktop fork release ID differs')
    expect(desktop?.artifact?.assetId === 571106914, 'Desktop fork installer asset ID differs')
    ...
    expect(desktop?.installedExecutable?.sha256 === '3406cf490050168cc3b38f78c354bafabf7b7a05f3ded2331d82b55300f2bb88', 'installed Desktop fork executable digest differs')
    ...
    expect(desktop?.installedRuntimeDescriptor?.sha256 === 'f0de4a61ead7105c41f1576a2f01617908e80e214c6c5383c9b79a13d50a14d1', 'installed Desktop runtime descriptor digest differs')
    ...
    expect(desktop?.releaseChannel?.sequence === 12, 'Desktop fork release sequence differs')
    expect(desktop?.releaseChannel?.manifestRawSha256 === '724214036567ddea1d6fb79bbfd4daa6c87eada4c00022ef4b0fa9170d93ff59', 'Desktop fork manifest raw digest differs')
    expect(desktop?.releaseChannel?.manifestSha256 === '52a2f43210cd694c06ff38452353473fc0cea47ba758959c573d1fbb66324090', 'Desktop fork manifest self digest differs')
    expect(desktop?.releaseChannel?.buildReceipt?.sha256 === '09307d85f8679308c424a9ea98aba89b1a80d97e914ea6cba385d7265170666c', 'Desktop fork build receipt raw digest differs')
    expect(desktop?.releaseChannel?.buildReceipt?.receiptSha256 === 'ec0229f180feb40afcd1da652cabf750f68dce823f525fc243d215b9c27e4fc8', 'Desktop fork build receipt self digest differs')
    try {
