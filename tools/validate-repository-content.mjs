...
export function validateRepositoryContent(root = defaultRoot) {
  ...
  expect(artifact?.releaseCommit === '479340f965c5be7b4408e4f1e6c9dda6c421d37b', 'locked Copilot Release commit differs')
  ...
  if (forkDesktop) {
    expect(desktop?.version === '0.1.6-alpha.1.cloga.1', 'Desktop fork version differs')
    expect(desktop?.source?.releaseTag === 'dsh-desktop-v0.1.6-alpha.1.cloga.1', 'Desktop fork release tag differs')
    expect(desktop?.source?.commit === 'fae12b69dcd28413518f68b5770e40f8eb2ff730', 'Desktop fork source commit differs')
    expect(desktop?.source?.tree === '3ab1707c85ade70f7df5e33e95eb9991b53229f1', 'Desktop fork source tree differs')
    expect(desktop?.source?.pullRequest === 45, 'Desktop fork release-plan pull request differs')
    expect(desktop?.source?.reviewedHead === '6bf111473593dbd9b69cf50a2651b94172a75bac', 'Desktop fork reviewed head differs')
    expect(desktop?.artifact?.name === 'cloga-deepseek-harness-0.1.6-alpha.1.cloga.1-win-x64.exe', 'Desktop fork installer name differs')
    expect(desktop?.artifact?.url === 'https://github.com/cloga/deepseek-harness/releases/download/dsh-desktop-v0.1.6-alpha.1.cloga.1/cloga-deepseek-harness-0.1.6-alpha.1.cloga.1-win-x64.exe', 'Desktop fork installer URL differs')
    expect(desktop?.artifact?.sha256 === '9e645df11f8f48f82f1fad512a763a07791b75f1cafe1e354d5d8d2826f2fae6', 'Desktop fork installer digest differs')
    expect(desktop?.artifact?.size === 171301423, 'Desktop fork installer size differs')
    expect(desktop?.artifact?.releaseId === 390539601, 'Desktop fork release ID differs')
    expect(desktop?.artifact?.assetId === 569806165, 'Desktop fork installer asset ID differs')
    ...
    expect(desktop?.installedExecutable?.sha256 === 'ea8c801e8cef289772c5d616a25aeaa255a86b3471b7549ca958cb7708bcf41f', 'installed Desktop fork executable digest differs')
    ...
    expect(desktop?.installedRuntimeDescriptor?.sha256 === 'b388ddee840f7de08ac391d4faf7a526ebd40b3bfe0ced8525cf8ac2c9fab344', 'installed Desktop runtime descriptor digest differs')
    ...
    expect(desktop?.releaseChannel?.sequence === 11, 'Desktop fork release sequence differs')
    expect(desktop?.releaseChannel?.manifestRawSha256 === '20e546c5ca5cb9152d931be023e67de1472b7a6fd6e078f97806888d57ee0d32', 'Desktop fork manifest raw digest differs')
    expect(desktop?.releaseChannel?.manifestSha256 === '50fa6bf1af2ae94bf7e2a032ec00665760b989c7468e514e0005ed78a616e207', 'Desktop fork manifest self digest differs')
    expect(desktop?.releaseChannel?.buildReceipt?.sha256 === '402cfe07a52832e87faca6754cc7a8a1ed3258480966e973160eaa46b1e56b83', 'Desktop fork build receipt raw digest differs')
    expect(desktop?.releaseChannel?.buildReceipt?.receiptSha256 === 'c3b88001c30095ca470d8c6d19ae9f027ec1c6b755ff7ca4d1b73a9ff9476b79', 'Desktop fork build receipt self digest differs')
    try {
