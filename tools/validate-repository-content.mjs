...
import { verifyNativeReleaseEvidence } from './verify-native-desktop.mjs'
...
export function validateRepositoryContent(root = defaultRoot) {
  ...
  expect(artifact?.releaseCommit === '76d190aed688e073df930adb3c753d2a749519c9', 'locked Copilot Release commit differs')
  ...
  if (forkDesktop) {
    expect(desktop?.version === '0.1.6-alpha.1.cloga.14', 'Desktop fork version differs')
    expect(desktop?.source?.releaseTag === 'dsh-desktop-v0.1.6-alpha.1.cloga.14', 'Desktop fork release tag differs')
    expect(desktop?.source?.commit === 'a0f0144f4cddc90c45f8be93c61c0cd6cec470c7', 'Desktop fork source commit differs')
    expect(desktop?.source?.tree === '8e636d110dcbd2e9662deb6d36e6e75b3f3fb0b2', 'Desktop fork source tree differs')
    expect(desktop?.source?.pullRequest === 105, 'Desktop fork release-plan pull request differs')
    expect(desktop?.source?.reviewedHead === '1a070a3b8cc09950e0ebaa7c4d36001a4b5efa12', 'Desktop fork reviewed head differs')
    expect(desktop?.artifact?.name === 'cloga-deepseek-harness-0.1.6-alpha.1.cloga.14-win-x64.exe', 'Desktop fork installer name differs')
    expect(desktop?.artifact?.url === 'https://github.com/cloga/deepseek-harness/releases/download/dsh-desktop-v0.1.6-alpha.1.cloga.14/cloga-deepseek-harness-0.1.6-alpha.1.cloga.14-win-x64.exe', 'Desktop fork installer URL differs')
    expect(desktop?.artifact?.sha256 === '81d73c63541d3b4464e27f1bae1651a3a996dfea4eaf89cc094f70c77b6eea0e', 'Desktop fork installer digest differs')
    expect(desktop?.artifact?.size === 171318565, 'Desktop fork installer size differs')
    expect(desktop?.artifact?.releaseId === 392640510, 'Desktop fork release ID differs')
    expect(desktop?.artifact?.assetId === 577940485, 'Desktop fork installer asset ID differs')
    ...
    expect(desktop?.installedExecutable?.sha256 === 'ff93b2388818f3b935e1bf0433117070ff27d290b26d465a7c8fd0be3f784608', 'installed Desktop fork executable digest differs')
    ...
    expect(desktop?.installedRuntimeDescriptor?.sha256 === '8cee8fcf841cc28235bc561aa89082e910bfc648cbfc953f735910bc19bfbe55', 'installed Desktop runtime descriptor digest differs')
    ...
    expect(desktop?.releaseChannel?.sequence === 24, 'Desktop fork release sequence differs')
    expect(desktop?.releaseChannel?.manifestRawSha256 === '3516a6062010f1adf139f9ce0874e2d2460f6a7883344f1cae78702200995767', 'Desktop fork manifest raw digest differs')
    expect(desktop?.releaseChannel?.manifestSha256 === '00fc3d57bab93f76ac7321ee6273a862a30765db1a9ebe385dd2781d84def5b8', 'Desktop fork manifest self digest differs')
    expect(desktop?.releaseChannel?.buildReceipt?.sha256 === '4525cf68029b8f14cc62c984d91215b698125feb6d4a45191db548c979f7ab4d', 'Desktop fork build receipt raw digest differs')
    expect(desktop?.releaseChannel?.buildReceipt?.receiptSha256 === '3af03c005e576c05e118d16b374864ce5ea50514d7cfa2d6ee61ee9ae33b7b9b', 'Desktop fork build receipt self digest differs')
    try {
      verifyNativeReleaseEvidence(lock, path.join(root, desktop.releaseChannel.nativeProvisioning.fixtureRoot.replaceAll('\\', '/')))
    } catch (error) {
      failures.push(`Desktop formal release evidence failed: ${error.message}`)
    }
  } else {
  ...
  if (forkDesktop) {
    ...
    expect(fork?.package?.version === '0.1.6-alpha.1', 'Desktop fork nested CLI version differs')
  ...
  if (forkDesktop) {
    ...
    if (/^0\.1\.6(?:-|$)/.test(desktop?.releaseChannel?.upstreamVersion ?? '')) {
      expect(desktop?.installedRuntimeDescriptor?.relativePath?.replaceAll('\\', '/') === 'resources/app.asar/dsh/desktop-runtime.json', 'ASAR descriptor path must be canonical')
      expect(/resources[\\/]app\.asar[\\/]dsh$/.test(runtimeSchema?.root ?? ''), 'ASAR runtime schema root must name the virtual runtime')
      expect(runtimeSchema?.layout === 'electron-asar', 'ASAR runtime schema layout is missing')
      expect(!Object.hasOwn(runtimeSchema ?? {}, 'wrapper'), 'ASAR runtime schema must not claim a legacy physical wrapper')
      expect(runtimeSchema?.descriptorSha256 === desktop?.installedRuntimeDescriptor?.sha256, 'ASAR runtime schema descriptor digest differs')
      expect(runtimeSchema?.package?.name === selector?.package?.name && runtimeSchema?.package?.version === selector?.package?.version, 'ASAR runtime package metadata differs')
      expect(runtimeSchema?.package?.manifest?.replaceAll('\\', '/') === 'node_modules/@deepseek-ai/dsh/package.json' &&
        runtimeSchema?.package?.entrypoint?.replaceAll('\\', '/') === 'node_modules/@deepseek-ai/dsh/lib/bin.js', 'ASAR runtime package paths differ')
      expect(Number.isSafeInteger(runtimeSchema?.package?.entrypointSize) && runtimeSchema.package.entrypointSize > 0 &&
        /^[a-f0-9]{64}$/.test(runtimeSchema?.package?.entrypointSha256 ?? ''), 'ASAR runtime entry data identity is invalid')
      const expectedNames = ['dsh-tools', 'dsh-tool-pwsh', 'dsh-sandbox'].map(name => `node_modules/@deepseek-ai/${name}/lib/index.js`)
      const files = runtimeSchema?.requiredBuiltFiles ?? []
      expect(files.length === expectedNames.length && expectedNames.every(name => files.filter(file =>
        file.path?.replaceAll('\\', '/') === name && Number.isSafeInteger(file.size) && file.size > 0 && /^[a-f0-9]{64}$/.test(file.sha256 ?? '')).length === 1),
      'ASAR runtime built-file data inventory is invalid')
    }
  } else {
